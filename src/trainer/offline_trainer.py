from typing import Iterable, Union, Optional, List, Any, Mapping, Dict, Tuple, Literal
import os
...
class OfflineRLTrainer:
    ...
    def __init__(
        ...
        self._current_train_return: Optional[Union[torch.Tensor, Mapping[str, Any]]] = {}
    ...
    def fit(
        ...
        optimizer, scheduler_cfg = self._parse_optimizers_schedulers(policy.configure_optimizers())
    ...
    def train_loop(
        ...
        for batch_idx, batch in enumerate(iterable):
            ...
            self.step_scheduler(policy, scheduler_cfg, level='step', current_value=self.current_step)
    ...
    def env_rollout(
        ...
        num_rollouts = self.num_test_rollouts if phase_is_test else self.num_val_rollouts
        assert num_rollouts % (env.num_envs * step_size) == 0
        
        total_rewards = np.zeros(env.num_envs)
        for i in self.probar_wrapper(range(num_rollouts, step=step_size), total=num_rollouts):
