...
class RTGEnvironment(gym.Env):
    ...
    def __init__(
        ...
        if not base_rtg.shape == ():
            raise ValueError(f"base_rtg must be a scalar or a 1D tensor. Got shape {base_rtg.shape}")
    ...
    def reset(self) -> torch.Tensor:
        ...
        self.actions = torch.empty((0, self.env.action_space.shape[0]), dtype=torch.float32)
    ...
    def _concat_to_context(self, new_obs: np.array, new_act: torch.tensor, new_reward: float):
        self.obs = torch.cat((self.obs, torch.from_numpy(new_obs).float().unsqueeze(0)), dim=0)
        self.actions = torch.cat((self.actions, new_act.unsqueeze(0)), dim=0)
        
        current_rtg = (self.rtgs[-1] - torch.tensor([new_reward], dtype=torch.float32)).to(self.rtgs.device)
        self.rtgs = torch.cat((self.rtgs, current_rtg), dim=0)
        
        self.timesteps = torch.cat((self.timesteps, torch.tensor([self.current_timestep], dtype=torch.float32)), dim=0)
